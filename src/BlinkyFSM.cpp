...
BlinkyFSM::BlinkyFSM(std::shared_ptr<Character> character): 
...
Move BlinkyFSM::getMove(const GameState& game)
{
...
Move NonFrightenedState::onUpdate(const GameState& gs)
{
    ...
    //CRUISE ELROY: cancela scatter y perseguir siempre a pacman
    if(elroy && activeSubState == scatter)
    {
        ...
        std::cout << "CRUISE ELROY: FORCE CHASE\n";
    }
